...
def main_i():
    ...
    op.add_option('--state-socket',       metavar='SOCKF', type=str, action='callback', callback=store_abs)
    ...
    if confdata:
        ...
        if confdata.opt == 'checkpoint' and not confdata.rx and \
           confdata.op in ('set', 'del'):
